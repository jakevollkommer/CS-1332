...
public class StringSearching {
    ...
    public static List<Integer> kmp(CharSequence pattern, CharSequence text) {
        if (pattern == null) {
            throw new IllegalArgumentException("Patten cannot be null");
        }
        if (pattern.length() == 0) {
            throw new IllegalArgumentException("Pattern must be longer than 0");
        }
        if (text == null) {
    ...
    public static List<Integer> boyerMoore(CharSequence pattern,
        ...
        while (i < m) {
            Character targ = text.charAt(i);
            if (targ == pattern.charAt(j)) {
                if (j == 0) {
                    matches.add(i);
                    j = n - 1;
                    i = i + n;
                ...
                i = i + (n - Math.min(j, 1 + last.getOrDefault(targ, -1)));
                j = n - 1;
            }
        }
        return matches;
    ...
    public static List<Integer> rabinKarp(CharSequence pattern,
        ...
        for (int i = 0; i <= text.length() - pattern.length(); i++) {
            if (tHash == pHash) {
                ...
                while (matchExists && j < pattern.length()) {
    ...
    public static int generateHash(CharSequence current, int length) {
        ...
        if (length <= 0 || length > current.length()) {
            throw new IllegalArgumentException("Length must be positive"
                    + " and less than the current length");
        }
        int hash = 0;
    ...
    public static int updateHash(int oldHash, int length, char oldChar,
        ...
        int hash = BASE * (oldHash - (int) oldChar * pow(BASE, length - 1))
            + (int) newChar;
        return hash;
    }
